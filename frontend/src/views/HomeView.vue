    ...
    <section class="insight-grid">
      <section class="config-stage compact-stage">
        <div class="config-stage-head">
          <div class="config-stage-heading">
            <div class="config-stage-title-row">
              <h3>任务调度配置</h3>
              <span class="workspace-pill" :class="cfstJobActive ? 'success' : 'danger'">
        ...
        <div v-if="jobs.length === 0" class="workspace-empty config-workflow-empty">
            ...
            <span>当前还没有保存任何调度任务配置</span>
        </div>

        <div v-else class="workflow-stack config-workflow-stack">
          <article class="workflow-node" v-for="job in jobs" :key="job.job_id">
            ...
            <div class="workflow-node-body">
              <div class="workflow-node-top">
                <h4>{{ job.name }}</h4>
                <span class="mono-text workflow-next-run">{{ formatNextRun(job.next_run) }}</span>
              </div>
        ...
        <form @submit.prevent="saveSchedulerSettings" class="config-stage-body compact-stage-body">
          <div class="config-editor-shell">
            ...
            <div class="config-field">
              ...
              <CronInput v-model="scheduleForm.cron_expr" />
              <small>
                默认值：<span class="mono-text">0 0 * * *</span>，表示每天零点执行。
              </small>
            </div>
            <button type="submit" class="save-config-btn" :disabled="savingScheduler">
              <span>
                <span v-if="savingScheduler" class="spinner-border spinner-border-sm me-2"></span>
      ...
      <article class="workspace-card workspace-card-side">
        ...
        <div class="action-tile-list">
          <button class="action-tile action-tile-primary" @click="runIpOptimization" :disabled="runningOptimization">
            <i class="bx bx-rocket"></i>
            <div>
              <strong>运行 IP 优选</strong>
              <span>{{ runningOptimization ? '正在执行 Cloudflare IP 优选与当前 IP 回写' : '执行 CFST 优选，并同步刷新 Tracker 当前 IP' }}</span>
            </div>
          </button>
          ...
          <button class="action-tile action-tile-success" @click="updateHosts" :disabled="updatingHosts">
            ...
            <div>
              ...
              <span>{{ updatingHosts ? '正在合并源内容并刷新目标 Hosts 文件' : '保留既有项目分区，仅按当前源重新合并写入' }}</span>
            </div>
          </button>
          ...
          <button class="action-tile action-tile-danger" @click="handleClearAndUpdate" :disabled="clearing">
            ...
            <div>
              <strong>清空并重建</strong>
              <span>{{ clearing ? '正在清空项目分区并重新生成 Hosts 内容' : '先清空项目写入分区，再重新执行完整 Hosts 生成流程' }}</span>
            </div>
          </button>
        </div>
...
const formatNextRun = (isoTime?: string | null): string => formatDateTimeCompact(isoTime, '未安排');
...
const CFST_JOB_ID = 'cfst';
const DEFAULT_CRON_EXPR = '0 0 * * *';
const CFST_JOB_NAME = 'CFST 测速任务';

const schedulerRunning = ref(false);
const jobs = ref<SchedulerJob[]>([]);
const runningOptimization = ref(false);
...
const cfstJob = computed(() => jobs.value.find((job) => job.job_id === CFST_JOB_ID) || null);
const cfstJobActive = computed(() => Boolean(cfstJob.value?.enabled));
const cfstJobStatusText = computed(() => {
  if (!schedulerRunning.value) return '调度器未启动';
  if (!cfstJob.value) return '未配置';
  if (!cfstJob.value.enabled) return '已停用';
  if (cfstJob.value.status === 'scheduled') return '已启用';
  if (cfstJob.value.status === 'running') return '执行中';
  return '待调度';
});
const applyCfstJob = (job?: SchedulerJob | null) => {
  scheduleForm.enabled = job?.enabled ?? true;
  scheduleForm.cron_expr = job?.cron_expr || DEFAULT_CRON_EXPR;
};
...
const fetchStatus = async () => {
  try {
    const [statusResponse, cfstJobResponse] = await Promise.all([
      scheduler.status(),
      scheduler.getJob(CFST_JOB_ID).catch(() => null),
    ]);
    schedulerRunning.value = Boolean(statusResponse.data?.running);
    jobs.value = Array.isArray(statusResponse.data?.jobs) ? statusResponse.data.jobs : [];
    applyCfstJob(cfstJobResponse?.data || cfstJob.value);
  } catch (e) {
    console.error('Failed to fetch status', e);
  }
};

const runIpOptimization = async () => {
  runningOptimization.value = true;
  try {
    const response = await cfst.run();
    const message = response.data?.message || 'Cloudflare IP 优选后台任务已启动';
    toast.info(message);
    setTimeout(fetchStatus, 2000);
  } catch (e: any) {
    const message = getErrorMessage(e, '启动失败');
    toast.error(`启动失败：${message}`);
  } finally {
    runningOptimization.value = false;
  }
};
...
const updateHosts = async () => {
  ...
  try {
    const response = await hostsStore.refreshHosts(false);
    const message = response?.data?.message || 'Hosts 刷新后台任务已启动';
    toast.info(message);
  } catch (e: any) {
    const message = getErrorMessage(e, '刷新失败');
...
const handleClearAndUpdate = async () => {
  ...
  clearing.value = true;
  try {
    const response = await hostsStore.rebuildHosts(true);
    const message = response?.data?.message || 'Hosts 清空重建后台任务已启动';
    toast.info(message);
  } catch (e: any) {
    const message = getErrorMessage(e, '操作失败');
...
const saveSchedulerSettings = async () => {
  ...
  try {
    await scheduler.createJob({
      ...
      cron_expr: scheduleForm.cron_expr || DEFAULT_CRON_EXPR,
    });
...
.quick-feedback-panel.is-success {
...
.quick-feedback-title-row h4,
.quick-feedback-detail h5 {
  margin: 0;
  color: var(--text-heading);
  font-size: 0.98rem;
  font-weight: 700;
...
.quick-feedback-head p,
.quick-feedback-detail p {
  margin: 0.38rem 0 0;
  color: var(--text-muted);
  line-height: 1.6;
}

.quick-feedback-badge {
  display: inline-flex;
  align-items: center;
  justify-content: center;
  min-height: 1.7rem;
...
.quick-feedback-badge.success {
  color: var(--success-color);
  background: rgba(74, 179, 126, 0.14);
}

.quick-feedback-badge.error {
  color: var(--danger-color);
  background: rgba(225, 108, 108, 0.14);
}

.quick-feedback-dismiss {
  flex: 0 0 auto;
  display: inline-flex;
  align-items: center;
  justify-content: center;
...
.quick-feedback-dismiss:hover,
.quick-feedback-dismiss:focus-visible {
  background: rgba(var(--primary-rgb), 0.1);
  color: var(--primary-color);
...
.quick-feedback-meta-item span {
  color: var(--text-muted);
  font-size: 0.76rem;
}

.quick-feedback-meta-item strong {
  color: var(--text-heading);
  font-size: 0.92rem;
  line-height: 1.45;
}

.quick-feedback-detail {
  margin-top: 0.9rem;
